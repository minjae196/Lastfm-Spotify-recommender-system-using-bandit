...
# ─────────────────────────────────────────────
# 1. 나만의 추천: 1곡씩 추천 & 피드백 즉시 반영
# ─────────────────────────────────────────────
if option == "나만의 추천":
    if st.button("🎯 개인화 추천 받기"):
        st.session_state.feedback = {}
        tracks = st.session_state.recommender.recommend_personal_top(top_k=1)
        st.session_state.tracks = []
    ...
    if st.session_state.tracks:
        track = st.session_state.tracks[0]
        st.subheader("🎯 나만의 추천 곡")
        ...
        choice = st.radio("이 곡은 어떤가요?", ["보류", "좋아요", "싫어요"], index=0, horizontal=True)
